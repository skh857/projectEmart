...
import com.emart.service.BasketService;
...
public class BasketController {
	...
	public String goodsAdd(GoodsDTO goodsDTO, Integer b_purchase_cnt, Model model, HttpSession session, HttpServletRequest request) {
		...
		try {
			...
			if(basketService.selectBasket(basketDTO) != 0) {
				...
				model.addAttribute("msg", "상품이 " + b_purchase_cnt + "개 중복 추가되었습니다.");
				model.addAttribute("url", "/emart/basket/list");
			...
			model.addAttribute("msg", "상품이 " + b_purchase_cnt + "개 추가되었습니다.");
			model.addAttribute("url", "/emart/basket/list");
			model.addAttribute("id", basketDTO.getId());
